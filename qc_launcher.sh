...
population_file_path='/nfs/users/nfs_m/mc14/lustre_home/GENOTIPI/COMPARISON/VBSEQ_QC/EUR/EUR.23chr.snps.vcf'
...
do
	...
	cat <(echo $population_head) <(egrep "^${chr}	" $population_file_path) > 'EUR_chr'${chr}'.snps.csv'
	sed -i 's/ /\t/g' 'EUR_chr'${chr}'.snps.csv'
	mv *.snps.csv SNPS_FILES/
	bsub -J "QC_script_chr_${chr}" -o "%J_chr${chr}_out.log"  -M8000000 -R"select[mem>8000] rusage[mem=8000]" \
	-q basement R CMD BATCH "--args SNPS_FILES/VBI_chr${chr}.snps.csv SNPS_FILES/EUR_chr${chr}.snps.csv ${chr}" /nfs/users/nfs_m/mc14/lustre_home/GENOTIPI/COMPARISON/VBSEQ_QC/scripts/esgi_vbseq_qc.r 
done

#bsub -J "QC_script_chr_[1-22]" -o "%J_chr%I_out.log" -M8000000 -R"select[mem>8000] rusage[mem=8000]" \
#-q basement 'R CMD BATCH "--args SNPS_FILES/VBI_chr"${LSB_JOBINDEX}".snps.csv SNPS_FILES/EUR_chr"${LSB_JOBINDEX}".snps.csv "${LSB_JOBINDEX}"" /nfs/users/nfs_m/mc14/lustre_home/GENOTIPI/COMPARISON/VBSEQ_QC/scripts/esgi_vbseq_qc.r'
...
create_plotter_file "VBI_chr" "EUR_chr" "_plot_table_MAF.csv" "VBI_snps_chr" "_table.vcs"> /nfs/users/nfs_m/mc14/lustre_home/GENOTIPI/COMPARISON/VBSEQ_QC/scripts/plotter.R

#bsub -J "remover_job" -w "numended(QC_script_chr,*)" -o "remover_job_out.log" -M8000000 -R"select[mem>8000] rusage[mem=8000]" \
#-q basement `rm VBI_chr*.snps.csv EUR_chr*.snps.csv`
#extract jobarray id
#jarrayid=`bjobs -q basement -A | cut -f 1 -d " " | tail -n 1`

#bsub -J "QC_plot" -w "numdone($jarrayid,22)" -o "QC_plot_out.log" -M16000000 -R"select[mem>16000] rusage[mem=16000]" \
#-q basement R CMD BATCH /nfs/users/nfs_m/mc14/lustre_home/GENOTIPI/COMPARISON/VBSEQ_QC/scripts/plotter.R

#bsub -J "QC_plot" -o "%J_QC_plot_out.log" -M16000000 -R "select[mem>16000] rusage[mem=16000]" \
#-q basement R CMD BATCH /nfs/users/nfs_m/mc14/lustre_home/GENOTIPI/COMPARISON/VBSEQ_QC/scripts/plotter.R

#bsub -J "move_job" -w "ended(QC_plot)" -o "move_job_out.log" -M8000000 -R"select[mem>8000] rusage[mem=8000]" \
#-q basement "mv *.pdf PDF/;mv *.jpg JPG/;mv multiallelic_chr*_table.csv MULTIALLELIC/;mv *mono_RR_chr* MONO/;mv *mono_AA_chr* MONO/;mv *plot_table_MAF* OVERLAP/PLOT_TABLES/;mv VBI_EUR_diff_rsIDs* OVERLAP/DIFFS/;mv VBI_EUR_all_overlap* OVERLAP/COMPLETE_OVERLAP/;mv *_summary* SUMMARIES/;mv VBI_snps_chr*_table.vcs SNPS_FILES/VBI_SNPS_NO_MULTI/"
