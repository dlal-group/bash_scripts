...
#First step:
#SNPs exclusion:exclude all snps not used in the analyses

bsub -J "s1_${trait}_chr${chr}" -o "$out_path/LOGS/%J_s1_chr${chr}.log" -e "$out_path/LOGS/%J_s1_chr${chr}.err" -G "team151" -M8000000 -R"select[mem>8000] rusage[mem=8000]" -q basement qctool -g $imp_file_path/CHR${chr}/chr${chr}.gen.gz -s $sample_file_path -incl-rsids $rs_to_remove_path -og $out_path/chr${chr}_snp_filtered.gen.gz

#Second step:
#Samples exclusion

bsub -J "s2_${trait}_chr${chr}" -w "ended(s1_${trait}_chr${chr})" -o "$out_path/LOGS/%J_s2_chr${chr}.log" -e "$out_path/LOGS/%J_s2_chr${chr}.err" -G "team151" -M8000000 -R"select[mem>8000] rusage[mem=8000]" -q basement qctool -g $out_path/chr${chr}_snp_filtered.gen.gz -s $sample_file_path -excl-samples $sample_to_remove_path -og $out_path/chr${chr}_sample_filtered.gen.gz -os $out_path/Filtered_chr${chr}_samples.sample
...
bsub -J "s6_${trait}_chr${chr}" -w "ended(s3_${trait}_chr${chr})" -o "$out_path/LOGS/%J_s6_chr${chr}.log" -e "$out_path/LOGS/%J_s6_chr${chr}.err" -G "team151" -q normal "rm $out_path/chr${chr}_sample_filtered.gen.gz"
