...
# mkdir -p LOGS;size=`wc -l /lustre/scratch113/projects/fvg_seq/07082015/BRIDGED/new_batch_misaligned_files_1.list|cut -f 1 -d " "`;bsub -J "bridge[1-${size}]" -o "LOGS/%J_bridge.%I.o" -q normal -- ~/Work/bash_scripts/ja_runner_par.sh -s ~/Work/bash_scripts/bridgebuilder_launcher.sh /lustre/scratch113/projects/fvg_seq/07082015/BRIDGED/new_batch_misaligned_files_1.list /lustre/scratch113/projects/fvg_seq/07082015/BRIDGED 011
# mkdir -p LOGS;size=`wc -l /lustre/scratch113/projects/fvg_seq/07082015/BRIDGED/new_batch_misaligned_files_3.list|cut -f 1 -d " "`;bsub -J "bridge[1-${size}]" -o "LOGS/%J_bridge.%I.o" -q normal -- ~/Work/bash_scripts/ja_runner_par.sh -s ~/Work/bash_scripts/bridgebuilder_launcher.sh /lustre/scratch113/projects/fvg_seq/07082015/BRIDGED/new_batch_misaligned_files_3.list /lustre/scratch113/projects/fvg_seq/07082015/BRIDGED 111
...
case $MODE in
	...
	echo "###############"
	#2.1 Run binnie
	bsub -J "binnie_bridge_builder_s$(basename ${file})" -o "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).log" -e "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).err" -M8000 -R"select[mem>8000] rusage[mem=8000]" -q basement -- ~/Work/bash_scripts/bridgebuilder_binnie.sh ${file} ${out_dir}
	...
	011 )
	#Assuming we only need to perform last two stages
	echo "###############"
	echo "#Binnie STAGE #"
	echo "###############"
	#2.1 Run binnie
	bsub -J "binnie_bridge_builder_s$(basename ${file})" -o "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).log" -e "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).err" -M8000 -R"select[mem>8000] rusage[mem=8000]" -q long -- ~/Work/bash_scripts/bridgebuilder_binnie.sh ${file} ${out_dir}
	# bsub -J "binnie_bridge_builder_s$(basename ${file})" -o "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).log" -e "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).err" -M9000 -R"select[mem>9000] rusage[mem=9000]" -q basement -- ~/Work/bash_scripts/bridgebuilder_binnie.sh ${file} ${out_dir}
	...
	echo "###############"
	#2.1 Run binnie
	bsub -J "binnie_bridge_builder_s$(basename ${file})" -o "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).log" -e "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).err" -M8000 -R"select[mem>8000] rusage[mem=8000]" -q normal -- ~/Work/bash_scripts/bridgebuilder_binnie_fix.sh ${file} ${out_dir}
	# bsub -J "binnie_bridge_builder_s$(basename ${file})" -o "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).log" -e "${out_dir}/%J_bridge_builder_binnie_s$(basename ${file}).err" -M9000 -R"select[mem>9000] rusage[mem=9000]" -q basement -- ~/Work/bash_scripts/bridgebuilder_binnie.sh ${file} ${out_dir}
