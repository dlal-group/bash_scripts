...
case $MODE in
  ...
  INGIROH )
    ...
    do
      ...
      case $pop in
        ...
        if [[ $mac -eq 1 ]]; then
          #statements
        bsub -J"roh_${pop}_${CHR}" -o"LOGS/%J_roh_${pop}_${CHR}.o" -q normal -M7000 -n16 -R"span[hosts=1] select[mem>=7000] rusage[mem=7000]" -- java -Xms5000m -Xmx5000m -jar /nfs/team151/software/IBDseq/ibdseq.r1206.jar gt=${pop_path}/${CHR}.vcf.gz nthreads=16 ibdlod=1 r2max=${r2} out=${outdir}/${pop}.roh
        else
        bsub -J"roh_${pop}_${CHR}" -o"LOGS/%J_roh_${pop}_${CHR}.o" -q normal -M7000 -n16 -R"span[hosts=1] select[mem>=7000] rusage[mem=7000]" -- java -Xms5000m -Xmx5000m -jar /nfs/team151/software/IBDseq/ibdseq.r1206.jar gt=${pop_path}/${CHR}.vcf.gz nthreads=16 ibdlod=1 r2max=${r2} minalleles=${mac} out=${outdir}/${pop}.roh
  ...
  INGIROHFILT )
    ...
    ;;
    LISTROH )
    echo "Calculate ROH for a list of samples from splitted vcf files with IBDseq!!"
    # echo "We'll have also data separate for villages in FVG"
    ...
    # pops_updated="FVG VBI CARL Erto Resia Illegio Sauris"
        ...
        if [[ $mac -eq 1 ]]; then
          #statements
        bsub -J"roh_${pop}_${CHR}" -o"LOGS/%J_roh_${pop}_${CHR}.o" -q normal -M5000 -n8 -R"span[hosts=1] select[mem>=5000] rusage[mem=5000]" -- java -Xms5000m -Xmx5000m -jar /nfs/team151/software/IBDseq/ibdseq.r1206.jar gt=${pop_path} excludesamples=${samples_to_exclude} chrom=${CHR} nthreads=8 ibdlod=1 r2max=${r2} out=${outdir}/${pop}.roh
        else
        bsub -J"roh_${pop}_${CHR}" -o"LOGS/%J_roh_${pop}_${CHR}.o" -q normal -M5000 -n8 -R"span[hosts=1] select[mem>=5000] rusage[mem=5000]" -- java -Xms5000m -Xmx5000m -jar /nfs/team151/software/IBDseq/ibdseq.r1206.jar gt=${pop_path} excludesamples=${samples_to_exclude} chrom=${CHR} nthreads=8 ibdlod=1 r2max=${r2} minalleles=${mac} out=${outdir}/${pop}.roh
