...
case $MODE in
  ...
  INGIROHFILT )
    ...
    # pops_updated="FVG VBI CARL Erto Resia Illegio Sauris"
        ...
        if [[ $mac -eq 1 ]]; then
          #statements
        bsub -J"roh_${pop}_${CHR}" -o"LOGS/%J_roh_${pop}_${CHR}.o" -q long -M6000 -n16 -R"span[hosts=1] select[mem>=6000] rusage[mem=6000]" -- java -Xms5000m -Xmx5000m -jar /nfs/team151/software/IBDseq/ibdseq.r1206.jar gt=${pop_path} excludesamples=${samples_to_exclude} chrom=${CHR} nthreads=16 ibdlod=1 r2max=${r2} out=${outdir}/${pop}.roh
        else
        bsub -J"roh_${pop}_${CHR}" -o"LOGS/%J_roh_${pop}_${CHR}.o" -q long -M6000 -n16 -R"span[hosts=1] select[mem>=6000] rusage[mem=6000]" -- java -Xms5000m -Xmx5000m -jar /nfs/team151/software/IBDseq/ibdseq.r1206.jar gt=${pop_path} excludesamples=${samples_to_exclude} chrom=${CHR} nthreads=16 ibdlod=1 r2max=${r2} minalleles=${mac} out=${outdir}/${pop}.roh
