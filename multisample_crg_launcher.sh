...
do
	...
	do
		...
		case ${CALLER} in
			GATK)
			echo "echo \"bash multisample_crg_call.sh ${chr} ${reg_file} $1 ${OUTF} ${TYPE} ${OMODE}\" | qsub -N \"chr${chr}_${reg}_multicall\" -o \"${log}/chr${chr}_${reg}_multicall.o\" -e \"${log}/chr${chr}_${reg}_multicall.e\" -l h_rt=200:00:00 -l vf=40G -cwd -q xe-el6 -pe smp 8" > ${OUTF}/${chr}/.jobs/${chr}_${reg}_job.${TYPE}.sh
			;;
	...
	qsub -N "multicall_concat_chr${chr}_${TYPE}" -o "$2/LOGS/multicall_concat_chr${chr}_${TYPE}.o" -e "$2/LOGS/multicall_concat_chr${chr}_${TYPE}.e" -hold_jid "chr${chr}_*_multicall" -l h_rt=80:00:00 -l vf=16G -cwd -q xe-el6 /nfs/users/xe/ggirotto/max/scripts/bash_scripts/multisample_crg_reg_concat.sh $2 ${TYPE}
...
#check if we have all chr called and merged:
