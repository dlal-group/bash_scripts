...
mkdir -p ${output_file_path}/$index
cd ${output_file_path}/$index/

geno_file=${geno_files[$index]}
info_file=${info_files[$index]}
output_file=${output_file_path}/$index/${chr}

#Extract geno file
geno_file_name=`basename $geno_file`
gunzip -c ${geno_file} | awk '!($0 in a){a[$0];print}' > ${output_file_path}/$index/${geno_file_name}.gen


echo "Convert to mach..."
#script to launch with qsub impute 2 mach conversion
/software/bin/R CMD BATCH '--args '${chr}' '${geno_file_name}.gen' '${info_file}' '${sample_file}' '${output_file}'' /nfs/users/nfs_m/mc14/Work/r_scripts/impute2mach.R ${output_file_path}/$index/${chr}.Rout
