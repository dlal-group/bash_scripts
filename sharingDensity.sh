...
T=`head -n 1 $1 | awk '{a=substr($2,length($2)-1,2); if (a==".0" || a==".1") print 1; else print 0; }'`

if [ "$T" -eq "1" ]; then

cat $1 | sed 's/\.[01]//' | sed 's/\.[01]//'
...
| awk -v first=$first -v last=$last -v resolution=$resolution -v map=$MAP -v N=$N -v minDens=$minDens '
...
$10/$11>=minDens { #snp usati per calcolare ibd diviso lunghezza segmento
