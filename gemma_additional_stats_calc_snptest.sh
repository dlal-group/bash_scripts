...
bsub -J "s1_${trait}_chr${chr}" -o "$out_path/LOGS/%J_s1_chr${chr}.log" -e "$out_path/LOGS/%J_s1_chr${chr}.err" -G "team151" -M5000000 -R "select[mem>5000] rusage[mem=5000]" -q normal -- "fgrep -w -v -f $rs_to_include_path $imp_file_path/chr${chr}.gen_info | cut -f 2 -d ' ' | grep -v position > $out_path/rs_to_remove_${chr}.list"

#Second step:
#Stats extraction

bsub -J "s2_${trait}_chr${chr}" -o "$out_path/LOGS/%J_s2_chr${chr}.log" -e "$out_path/LOGS/%J_s2_chr${chr}.err" -w "ended(s1_${trait}_chr${chr})" -G "team151" -M8000000 -R "select[mem>8000] rusage[mem=8000]" -q normal -- /nfs/team151/software/bin/snptest_v2 -summary_stats_only -data $imp_file_path/chr${chr}.gen.gz $sample_file_path -exclude_snps $out_path/rs_to_remove_${chr}.list -exclude_samples $sample_to_remove_path -hwe -log $out_path/chr${chr}_filtered.log -o $out_path/${stat_filename}.${chr}.stats
