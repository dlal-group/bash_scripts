...
sub FormatChange{
	...
	$output =~ s/\.recode\.vcf/\.amelia_format\.txt/;
	...
	while(<INPUT>){
		...
		print OUTPUT "$array[0]\t$array[1]\t$array[2]\t$array[3]\t$array[4]\t$qual";
		...
		for ($i = 0; $i <= $format_last_index; $i++)
		  {
		    ...
		    if ($format_array[$i] eq 'GQ'){$qual_index = $i;}
		  }
		...
		for ($i = 9; $i <= $last_index; $i++)
		  {
		    ...
		    if ($qual_index eq ''){$genotype_qual = 100;}
		    else
		      {
			if (defined $genotype_info_array[$qual_index])
			  {
			    if ($genotype_info_array[$qual_index] eq '.'){$genotype_qual = 100;}
			    else                                         {$genotype_qual = $genotype_info_array[$qual_index];}
			  }#end if qual index defined
			else
			  {
			    $genotype_qual = 100;
			  }#end if this genotype doesn't have genotype quality defined
		      }#end else have a qual index

		    if ($genotype =~ /\|/)
		      {
			($first_allele,$second_allele) = split(/\|/,$genotype);
			print OUTPUT "\t$first_allele\t$second_allele\t$genotype_qual";
		      }
		    elsif ($genotype =~ /\//)
		      {
			($first_allele,$second_allele) = split(/\//,$genotype);
