...
bsub -J"extract_${mode}_acgt2dpgt5_${first_suffix}" -o"${outdir}/LOG/1.%J_extract_${mode}_acgt2dpgt5_${first_suffix}.o" -M 1000 -R "select[mem>=1000] rusage[mem=1000]" -q normal -- bcftools query -i"TYPE='${mode}' && AC>=2 && DP>=5" -f "%CHROM\t%POS\t%REF\t%ALT\n" ${vcf} -o ${outdir}/${filename}.${mode}_ac2dp5.tab
bsub -J"extract_${mode}_aceq0dpgt5_${first_suffix}" -o"${outdir}/LOG/2.%J_extract_${mode}_aceq0dpgt5_${first_suffix}.o" -M 1000 -R "select[mem>=1000] rusage[mem=1000]" -q normal -- bcftools query -i"TYPE='${mode}' && AC==0 && DP>=5" -f "%CHROM\t%POS\t%REF\t%ALT\n" ${vcf} -o ${outdir}/${filename}.${mode}_ac0dp5.tab

#select sites with AC=1 and DP>5
bsub -J"extract_${mode}_aceq1dpgt5_${first_suffix}" -o"${outdir}/LOG/3.%J_extract_${mode}_aceq1dpgt5_${first_suffix}.o" -M 1000 -R "select[mem>=1000] rusage[mem=1000]" -q normal -- bcftools query -i"TYPE='${mode}' && AC==1 && DP>=5" -f "%CHROM\t%POS\t%REF\t%ALT\n" ${vcf} -o ${outdir}/${filename}.${mode}_ac1dp5.tab
...
bsub -J"extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_INGI" -o"${outdir}/LOG/4.%J_extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_INGI.o" -w "ended(extract_${mode}_aceq1dpgt5_${first_suffix})" -M 2000 -R "select[mem>=2000] rusage[mem=2000]" -q normal --  python /nfs/users/nfs_m/mc14/Work/bash_scripts/overlap_check.py INGI ${outdir}/${filename}.${mode}_ac1dp5.tab /lustre/scratch113/projects/esgi-vbseq/16112015_TRIESTE/INGI/UNION/${chr}/sites.txt ${outdir} ${mode}

#extract a list of sites in common with 1000G or/and UK10K
bsub -J"extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_TGP3" -o"${outdir}/LOG/5.%J_extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_TGP3.o" -w "ended(extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_INGI)" -M 2000 -R "select[mem>=2000] rusage[mem=2000]" -q normal --  python /nfs/users/nfs_m/mc14/Work/bash_scripts/overlap_check.py TGP3 ${outdir}/${filename}.${mode}_ac1dp5.tab.INGI.${mode}.not_over.tab /lustre/scratch113/projects/esgi-vbseq/16112015_TRIESTE/TGPph3/UNION/${chr}/sites.txt ${outdir} ${mode}
bsub -J"extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_UK10K" -o"${outdir}/LOG/6.%J_extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_UK10K.o" -w "ended(extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_INGI)" -M 2000 -R "select[mem>=2000] rusage[mem=2000]" -q normal --  python /nfs/users/nfs_m/mc14/Work/bash_scripts/overlap_check.py UK10K ${outdir}/${filename}.${mode}_ac1dp5.tab.INGI.${mode}.not_over.tab /lustre/scratch113/projects/esgi-vbseq/16112015_TRIESTE/UK10K/UNION/${chr}/sites.txt ${outdir} ${mode}
