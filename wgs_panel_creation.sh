...
bsub -J"merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort}_tbi" -o"${outdir}/LOG_OVERLAPS/7.%J_merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort}_tbi.o" -w "ended(merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort})" -M 1000 -R "select[mem>=1000] rusage[mem=1000]" -q normal -- tabix -b 2 -e 2 -s 1 ${outdir}/${chr}/${filename}.${mode}_extract_annotation.list.gz

echo "bcftools annotate -a ${outdir}/${chr}/${filename}.${mode}_extract_annotation.list.gz -m REF_PANEL -c CHROM,POS,REF,ALT ${vcf} | bcftools view -i\"TYPE='${mode}' && INFO/REF_PANEL=1\" -O z -o ${outdir}/PANEL/${filename}.${mode}_REF.vcf.gz ${vcf}" | bsub -J"annotate_and_extract_ingi_${mode}_${first_suffix}_${cohort}_vcf" -o"${outdir}/LOG_PANEL/9.%J_annotate_and_extract_ingi_${mode}_${first_suffix}_${cohort}_vcf.o" -w"ended(merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort})" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal
bsub -J"annotate_and_extract_ingi_${mode}_${first_suffix}_${cohort}_vcf_tbi" -o"${outdir}/LOG_PANEL/8.%J_annotate_and_extract_ingi_${mode}_${first_suffix}_${cohort}_vcf_tbi.o" -w"ended(annotate_and_extract_ingi_${mode}_${first_suffix}_${cohort}_vcf)" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal -- tabix -p vcf ${outdir}/PANEL/${filename}.${mode}_REF.vcf.gz
;;
MERGE )
# add a bit for merg back SNP and INDEls
bsub -J"merge_${first_suffix}_${cohort}_vcf" -o"${outdir}/LOG_PANEL/9.%J_merge_${first_suffix}_${cohort}_vcf.o" -w"ended(annotate_and_extract_ingi_*_${first_suffix}_${cohort}_vcf_tbi)" -M 1000 -R "select[mem>=1000] rusage[mem=1000]" -q normal -- bcftools concat -a  ${outdir}/PANEL/${filename}.snp_REF.vcf.gz ${outdir}/PANEL/${filename}.indel_REF.vcf.gz -O z -o ${outdir}/PANEL/${filename}.ALL_REF.vcf.gz
bsub -J"merge_${first_suffix}_${cohort}_vcf_tbi" -o"${outdir}/LOG_PANEL/9.%J_merge_${first_suffix}_${cohort}_vcf_tbi.o" -w"ended(merge_${first_suffix}_${cohort}_vcf)" -M 1000 -R "select[mem>=1000] rusage[mem=1000]" -q normal -- tabix -p vcf ${outdir}/PANEL/${filename}.ALL_REF.vcf.gz
