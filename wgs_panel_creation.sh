...
bsub -J"annotate_ingi_${mode}_${first_suffix}_${cohort}_vcf" -o"${outdir}/LOG_PANEL/10.%J_annotate_ingi_${mode}_${first_suffix}_${cohort}_vcf.o" -w"ended(merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort})" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal -- bcftools annotate -a ${outdir}/${chr}/${filename}.${mode}_extract_annotation.list.gz -m REF_PANEL -c CHROM,POS,REF,ALT -O z -o ${outdir}/PANEL/${filename}.${mode}_annREF.vcf.gz ${vcf} 
bsub -J"annotate_ingi_${mode}_${first_suffix}_${cohort}_vcf_tbi" -o"${outdir}/LOG_PANEL/10.%J_annotate_ingi_${mode}_${first_suffix}_${cohort}_vcf_tbi.o" -w"ended(annotate_ingi_${mode}_${first_suffix}_${cohort}_vcf)" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal -- tabix -f -p vcf ${outdir}/PANEL/${filename}.${mode}_annREF.vcf.gz

bsub -J"extract_ingi_${mode}_${first_suffix}_${cohort}_vcf" -o"${outdir}/LOG_PANEL/11.%J_extract_ingi_${mode}_${first_suffix}_${cohort}_vcf.o" -w"ended(annotate_ingi_${mode}_${first_suffix}_${cohort}_vcf_tbi)" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal -- bcftools view -i"TYPE='${mode}' && INFO/REF_PANEL=1" -O z -o ${outdir}/PANEL/${filename}.${mode}_REF.vcf.gz ${outdir}/PANEL/${filename}.${mode}_annREF.vcf.gz 
bsub -J"extract_ingi_${mode}_${first_suffix}_${cohort}_vcf_tbi" -o"${outdir}/LOG_PANEL/11.%J_extract_ingi_${mode}_${first_suffix}_${cohort}_vcf_tbi.o" -w"ended(extract_ingi_${mode}_${first_suffix}_${cohort}_vcf)" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal -- tabix -f -p vcf ${outdir}/PANEL/${filename}.${mode}_REF.vcf.gz
;;
MERGE )
