...
bsub -J"extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_TGP3" -o"%J_extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_TGP3.o" -w "ended(extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_INGI)" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal --  python /nfs/users/nfs_m/mc14/Work/bash_scripts/overlap_check.py TGP3 ${outdir}/${filename}.${mode}_ac1dp5.tab.INGI.${mode}.not_over.tab /lustre/scratch113/projects/esgi-vbseq/16112015_TRIESTE/TGPph3/UNION/${chr}/sites.txt ${outdir} ${mode}
bsub -J"extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_UK10K" -o"%J_extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_UK10K.o" -w "ended(extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_INGI)" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal --  python /nfs/users/nfs_m/mc14/Work/bash_scripts/overlap_check.py UK10K ${outdir}/${filename}.${mode}_ac1dp5.tab.INGI.${mode}.not_over.tab /lustre/scratch113/projects/esgi-vbseq/16112015_TRIESTE/UK10K/UNION/${chr}/sites.txt ${outdir} ${mode}
...
echo " (cat \${outdir}/\${filename}.\${mode}_ac0dp5.tab \${outdir}/\${filename}.\${mode}_ac2dp5.tab <(cat \${outdir}/\${filename}.\${mode}_ac1dp5.tab.INGI.\${mode}.over.tab  \${outdir}/\${filename}.\${mode}_ac1dp5.tab.INGI.\${mode}.not_over.tab.TGP3.\${mode}.over.tab \${outdir}/\${filename}.\${mode}_ac1dp5.tab.INGI.\${mode}.not_over.tab.UK10K.\${mode}.over.tab | cut -f -4 )) | sort -g -k1,1 -k2,2 | uniq | cut -f -2 > \${outdir}/\${filename}.\${mode}_extract.list " | bsub -J"merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort}" -o"%J_merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort}.o" -w "ended(extract_common_ingi_${mode}_aceq1dpgt5_${first_suffix}_*)" -M 1000 -R "select[mem>=1000] rusage[mem=1000]" -q normal
...
bsub -J"generate_ingi_${mode}_${first_suffix}_${cohort}_vcf" -o"%J_generate_ingi_${mode}_${first_suffix}_${cohort}_vcf.o" -w"ended(merge_common_ingi_${mode}_aceq0eq1gt2dpgt5_${first_suffix}_${cohort})" -M 3000 -R "select[mem>=3000] rusage[mem=3000]" -q normal -- bcftools view -i"TYPE='${mode}'" -R ${outdir}/${filename}.${mode}_extract.list -O z -o ${outdir}/PANEL/${filename}.${mode}_REF.vcf.gz ${vcf}
