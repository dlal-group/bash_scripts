...
-q yesterday "plink --noweb --file $1 --recode --allow-no-sex --out ${outname_f1}"

bsub -J "recode_f2" -o "%J_recode_f2.log" -M1000 -R"select[mem>1000] rusage[mem=1000]" \
-q yesterday "plink --noweb --file $2 --recode --allow-no-sex --out ${outname_f2}"
...
-q yesterday "sed -i 's/^[ 	]*//;s/ \+/	/g' ${outname_f2}.frq"
...
-q yesterday "sort -k1 ${outname_f1}.ped > ${outname_f1}.sorted;mv ${outname_f1}.ped ${outname_f1}.unsorted;mv ${outname_f1}.sorted ${outname_f1}.ped;sort -k1 ${outname_f2}.ped > ${outname_f2}.sorted;mv ${outname_f2}.ped ${outname_f2}.unsorted;mv ${outname_f2}.sorted ${outname_f2}.ped"
...
-q yesterday R CMD BATCH "--args ${outname_f1}.frq ${outname_f1}.map" /nfs/users/nfs_m/mc14/Work/r_scripts/gt_discordance.r
fi
...
do
	...
	then
		...
		-q yesterday discordance_inner_cicle.sh ${outname_f1} ${outname_f2} ${chr} $3 $4
	else
		...
		-q yesterday discordance_inner_cicle.sh ${outname_f1} ${outname_f2} ${chr} $3 ref_discordance_table.txt
	fi
done

bsub -J "last_step" -w "ended(gt_calculator_chr*)" -o "%J_last_step.log" -M8000 -R"select[mem>8000] rusage[mem=8000]" \
-q yesterday last_discordance_pipeline_step.sh $3
