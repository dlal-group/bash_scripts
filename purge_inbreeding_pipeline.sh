...
case $MODE in
  ...
  pops_updated="FVG VBI TSI CEU CARL Erto Resia Illegio Sauris"
    ...
    do
      ...
      mkdir -p ${outdir}/EMI/w_20k/${pop}; emi ${outdir}/${pop}.ibd.length.5.ibd -fam ${outdir}/${pop}.ibd.length.5.fam -wgt 7th 5 50 -win 20000 bp ${outdir}/EMI/w_20k/${pop}/${pop}.out
      mkdir -p ${outdir}/EMI/w_200k/${pop}; emi ${outdir}/${pop}.ibd.length.5.ibd -fam ${outdir}/${pop}.ibd.length.5.fam -wgt 7th 5 50 -win 200000 bp ${outdir}/EMI/w_200k/${pop}/${pop}.out

      # format output file to sort cluster and remove duplicates:
      awk -F "\t" -v OFS="\t" '{$1="clst"; print $0}' ${outdir}/EMI/w_20k/${pop}/${pop}.out.clst.tmp|sort -k2n -k3n -k4n | uniq| awk '{$1="clst"NR;print $0}' > ${outdir}/EMI/w_20k/${pop}/${pop}.out.clst.uniq.sorted.txt
      awk -F "\t" -v OFS="\t" '{$1="clst"; print $0}' ${outdir}/EMI/w_200k/${pop}/${pop}.out.clst.tmp|sort -k2n -k3n -k4n | uniq| awk '{$1="clst"NR;print $0}' > ${outdir}/EMI/w_200k/${pop}/${pop}.out.clst.uniq.sorted.txt
  
      # Create a file with cluster positions and number of samples:
      awk -v chr=${CHR} '{print $1,chr,$2,$3,(NF-3)/2}' ${outdir}/EMI/w_20k/${pop}/${pop}.out.clst.uniq.sorted.txt > ${outdir}/EMI/w_20k/${pop}/${pop}.out.clst.uniq.size
      awk -v chr=${CHR} '{print $1,chr,$2,$3,(NF-3)/2}' ${outdir}/EMI/w_200k/${pop}/${pop}.out.clst.uniq.sorted.txt > ${outdir}/EMI/w_200k/${pop}/${pop}.out.clst.uniq.size
