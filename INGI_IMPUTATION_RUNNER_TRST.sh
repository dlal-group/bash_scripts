...
do
	...
	a_size=`wc -l $imputedir/chr${chr}_command.list| cut -f 1 -d " "`;echo "~/scripts/bash_scripts/ja_runner_TRST.sh -s $imputedir/chr${chr}_command.list"|qsub -t 1-${a_size} -o ${imputedir}/chr${chr}_\${JOB_ID}_\$TASK_ID.log -e ${imputedir}/chr${chr}_\${JOB_ID}_\$TASK_ID.e -V -N ${pop}_chr${chr} -l h_vmem=${m}
done
