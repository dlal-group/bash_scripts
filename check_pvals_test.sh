#!/usr/local/bin/bash
################ATTENTION!!!!!################################
#This script works with PLAIN FILES in GEMMA OUTPUT FORMAT!!##
##############################################################
...
then
	...
	(echo "chr rs pos p_vald -log_P exp_p -log_exp_p"|tr " " "\t";(zcat $1_chi2.gz | tail -n+2 | awk '{print $0,-(log($6)/log(10.0))}'|tr " " "\t"|sort -g -k6,6| awk -v l=$t '{m=l;a=0.5;print $0,((NR)-a)/(m+(1-a)-a),-(log(((NR)-a)/(m+(1-a)-a))/log(10.0))}'|tr " " "\t"|cut -f -3,6,8,9,10))> $1_chi2.plots
	...
	#ADD a statement to choose if calculate again the lambda
	# if [ $2=="--lambda"]
	# then
	...
	# 		(echo "rs chi2_gc";zcat $1_chi2.gz | tail -n+2 | awk -v lam=$lambda '{print $2, $7/lam}')| tr " " "\t" | gzip -c > $1_chi2_gc.gz
